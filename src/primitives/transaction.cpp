...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between synx and zsynx
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // SYNX: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
