...
#if defined(NDEBUG)
#error "SYNX cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SYNX
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if ((Params().NetworkID() != CBaseChainParams::REGTEST) && block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= 512)) {
    ...
    if (block.nBits != nBitsRequired) {
        // Syndicate Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().SyndicateBadBlockTime()) && (block.nBits == (uint32_t) Params().SyndicateBadBlocknBits())) {
            // accept SYNX block minted with incorrect proof of work threshold
            return true;
        }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SYNX: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
    // SPORK_14 is used for 70921 (v2.3.0)
    if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
		return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15  was used for 70920 (v2.2.0), commented out now.
